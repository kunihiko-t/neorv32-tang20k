...
作業はこの独立クローン内で完結させる。FPGA側は既存のNEORV32、内蔵IMEM/DMEM、UART0、内蔵ブートローダー構成を使う。ソフトウェア側は依存クレートを持たない`no_std`/`no_main`のRustバイナリとし、UART0（`0xFFF50000`）を直接操作する。参照実装の108MHz制約ではタイミングを満たさないため、CPUとUARTの基準クロックは96MHzとする。
