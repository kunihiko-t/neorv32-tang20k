...
**Architecture:** 既存のTang Nano 20KトップレベルとNEORV32ブートローダーは変更しない。依存クレートのないRustアプリを別ディレクトリに追加し、ホストテスト、クロスビルド、NEORV32 image generator、FPGA合成を順に検証する。
...
- [ ] **Step 1: Install the workspace-local FPGA toolchain**

  `oss-cad-suite-darwin-arm64-20260831.tgz`を`/Users/valletta/dev/tang_nano_20k/.tools/`へ展開する。Homebrewやシステム領域は変更しない。

- [ ] **Step 2: Add the Rust target**

  Run: `rustup target add riscv32imc-unknown-none-elf`

- [ ] **Step 3: Add the environment loader**
...
- [ ] **Step 3: Implement the minimal UART divisor calculation**
...
**Interfaces:**
- Consumes: UART0 base `0xFFF50000`、108MHz clock、19,200 baud
...
- [ ] **Step 3: Add the minimal bare-metal program**

  `main.rs`は`global_asm!`で`sp=0x80003f40`を設定して`rust_main`を呼ぶ。`rust_main`はUART0 CTRLへ`uart_control(108_000_000, 19_200)`を書き、TX FIFO full（CTRL bit 21）が解除されるのを待って`MiniOS/RV32 hello\r\n`をDATA（base+4）へ1 byteずつ書く。

- [ ] **Step 4: Add the linker and image build**
...
- [ ] **Step 1: Build the existing FPGA design**

  Run: `source script/env.sh && make bitstream summary`

  Expected: `build/top.fs`が生成され、タイミングとデバイス使用率のsummaryが終了コード0で出る。

- [ ] **Step 2: Document safe commands**
...
- [ ] **Step 3: Verify documentation commands**
  ...
  Run: `source script/env.sh && cargo test --manifest-path sw/minios_hello/Cargo.toml && bash sw/minios_hello/tests/check_image.sh && make bitstream summary`
